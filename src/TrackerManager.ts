...
export class TrackerManager {
	...
	constructor(context: vscode.ExtensionContext, metadataManager: MetadataManager,
        private readonly overlay: HiddenCodeOverlay) {
		this.context = context;
		this.metadataManager = metadataManager;
		this.hiddenCodeOverlay = overlay;
		console.log('[hidden-overlay][TrackerManager] constructed');
	}

	public async init(): Promise<void> {
		console.log('[hidden-overlay][TrackerManager] init()');
		this.registerInputInterceptors();

		// TODO: file system watchers (example)
		// const watcher = vscode.workspace.createFileSystemWatcher('**/*');
		// this.disposables.push(
		// 	watcher,
		// 	watcher.onDidChange(uri => this.handleFileDidChange(uri)),
		// 	watcher.onDidCreate(uri => this.handleFileDidCreate(uri)),
		// 	watcher.onDidDelete(uri => this.handleFileDidDelete(uri)),
		// );

		// TODO: terminal tracking, I/O logging, etc.
		// this.disposables.push(
		// 	vscode.window.onDidOpenTerminal(term => {
		// 		console.log('[hidden-overlay][TrackerManager] terminal opened:', term.name);
		// 	})
		// );

		// TODO: hook up anything like save events
		// this.disposables.push(
		// 	vscode.workspace.onDidSaveTextDocument(doc => {
		// 		console.log('[hidden-overlay][TrackerManager] saved:', doc.uri.fsPath);
		// 	})
		// );
		...
		this.disposables.push(
            vscode.workspace.onDidChangeTextDocument((event) => {
                ...
                const isDebugInsertion = this.overlay.getMode() === 'debugOn';
                // Update the per-character ledger with the granular edits
                this.metadataManager.handleTextDocumentChange(event.document, event.contentChanges, isDebugInsertion);

                // If in debug-on, update debug-char highlights (no text replacement)
                if (isDebugInsertion) {
                    this.overlay.updateHighlightsForDocument(event.document);
                }
            })
        );
		...
		this.disposables.push(
            vscode.window.onDidChangeActiveTextEditor((editor) => {
                ...
                if (this.overlay.getMode() === 'debugOn') {
                    this.overlay.updateHighlightsForDocument(editor.document);
                } else {
	...
	private registerInputInterceptors() {
        const reg = (cmd: string, handler: (args?: any) => Thenable<void> | void) => {
            this.disposables.push(vscode.commands.registerCommand(cmd, handler));
        };

        // Type a character
        reg('type', async (args: { text: string }) => {
            const editor = vscode.window.activeTextEditor;
            if (!editor || editor.document.uri.scheme !== 'file') {
                return vscode.commands.executeCommand('default:type', args);
            }
            const uri = editor.document.uri.toString();
            const selections = editor.selections.map(s => ({ start: s.start, end: s.end }));
            this.metadataManager.recordKeystroke({
                kind: 'type',
                text: args?.text ?? '',
                uri,
                selections,
                isDebug: this.overlay.getMode() === 'debugOn',
                timestamp: Date.now()
            });
            return vscode.commands.executeCommand('default:type', args);
        });
        ...
        // Paste
        reg('paste', async (args?: any) => {
            const ed = vscode.window.activeTextEditor;
            if (ed?.document.uri.scheme === 'file') {
                this.metadataManager.recordKeystroke({
                    kind: 'paste',
                    text: '', // content will arrive in onDidChangeTextDocument
                    uri: ed.document.uri.toString(),
                    selections: ed.selections.map(s => ({ start: s.start, end: s.end })),
                    isDebug: this.overlay.getMode() === 'debugOn',
                    timestamp: Date.now()
            ...
            return vscode.commands.executeCommand('editor.action.clipboardPasteAction', args);
        });

        // Cut
        reg('cut', async (args?: any) => {
            const ed = vscode.window.activeTextEditor;
            if (ed?.document.uri.scheme === 'file') {
                this.metadataManager.recordKeystroke({
                    kind: 'cut',
            ...
            return vscode.commands.executeCommand('editor.action.clipboardCutAction', args);
        });

        // Undo/Redo (optional)
        reg('undo', async () => vscode.commands.executeCommand('undo'));
        reg('redo', async () => vscode.commands.executeCommand('redo'));
    }
